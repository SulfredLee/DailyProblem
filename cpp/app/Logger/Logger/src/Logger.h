...
#define __LOGMSG(level, ...)                                            \
    do{                                                                 \
        char buffer[5120];                                              \
            _snprintf_s(buffer, sizeof(buffer), __VA_ARGS__);           \
            Logger::GetInstance().Log(level, "%25s::%25s:%6d,%6d,%s", Logger::GetInstance().GetClassName(this, __PRETTY_FUNCTION__).c_str(), __FUNCTION__, __LINE__, GetCurrentThreadID_(), buffer); \
    }while(0)
#else
#define __LOGMSG(level, ...)                                            \
    do{                                                                 \
...
#define __LOGMSG_C(level, ...)                                          \
    do{                                                                 \
        char buffer[5120];                                              \
        _snprintf_s(buffer, sizeof(buffer), __VA_ARGS__);               \
        Logger::GetInstance().Log(level, "%25s:%6d,%6d,%s", __FUNCTION__, __LINE__, GetCurrentThreadID_(), buffer); \
    }while(0)
#else
#define __LOGMSG_C(level, ...)                                          \
    do{                                                                 \
